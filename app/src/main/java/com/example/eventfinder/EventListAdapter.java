...
public class EventListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public EventListAdapter(RecyclerViewInterface recyclerViewInterface) {
        this.recyclerViewInterface = recyclerViewInterface;
    }
//    private String search_url;
    public EventListAdapter(Context mContext, List<EventListCard> mData) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, @SuppressLint("RecyclerView") int position) {
        ViewHolder eventHolder= (ViewHolder) holder;
        eventHolder.eventId=mData.get(position).getEventId();
        ...
        Glide.with(mContext).load(mData.get(position).getImageUrl()).into(eventHolder.event_image);
        ...
        System.out.println("here on bind " + favoritesList);
            ...
            for (int i = 0; i < favoritesList.length(); ++i) {
                try {
                    if (favoritesList.getJSONObject(i).getString("eventId").equals(mData.get(position).getEventId())) {
                        ...
                        eventHolder.favorite_icon.setImageResource(R.drawable.ic_favorites_fill);
                    }else{
                        mData.get(position).setFavorite(false);
                        eventHolder.favorite_icon.setImageResource(R.drawable.ic_favorites);
                    }
                } catch (JSONException ex) {
                    throw new RuntimeException(ex);
                }
...
//                        Snackbar.make("Item was removed from the list.", Snackbar.LENGTH_LONG).show();
...
//                    String StrJson=gson.toJson(favoritesList);
        ...
        eventHolder.cardView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.putExtra("eventName",mData.get(position).getName());
                intent.putExtra("eventFavorite",mData.get(position).getFavorite());
